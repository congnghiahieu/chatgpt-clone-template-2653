...
const Index = () => {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      } else if (content.toLowerCase().includes('top') || content.toLowerCase().includes('danh sách')) {
        assistantMessage = {
          role: 'assistant',
        ...
        <div className={`flex h-full flex-col ${messages.length === 0 ? 'items-center justify-center' : 'justify-between'} pt-[60px] pb-4`}>
          {messages.length === 0 ? (
            <div className="w-full max-w-3xl px-4 space-y-4">
              <div>
                <h1 className="mb-8 text-4xl font-semibold text-center">VPBank Text2SQL Assistant</h1>
                <p className="mb-6 text-center text-gray-400">Hỏi đáp thông minh về dữ liệu ngân hàng với phân quyền theo user</p>
