...
const Sidebar = ({ isOpen, onToggle, onChatSelect }: SidebarProps) => {
  ...
  return (
    <div
      className={cn(
        'fixed left-0 top-0 z-40 h-screen bg-chatgpt-sidebar transition-all duration-300',
        isOpen ? 'w-64' : 'w-0',
      )}
    >
      <nav
      ...
      >
        <div className='flex h-[60px] items-center justify-between'>
          <button
            onClick={onToggle}
            className='text-token-text-secondary hover:bg-token-sidebar-surface-secondary h-10 rounded-lg px-2'
          >
            <Menu className='h-5 w-5' />
          </button>
          <button className='hover:bg-token-sidebar-surface-secondary flex items-center gap-2 rounded-lg px-3 py-1 text-sm'>
            <svg
              width='24'
              height='24'
              viewBox='0 0 24 24'
